...
class SegBuilder(Builder):
    ...
    def lr_scheduler(self) -> paddle.optimizer.lr.LRScheduler:
        ...
        if lr_type == "PolynomialDecay":
            iters = (self.config.iters - warmup_iters
                     if use_warmup else self.config.iters)
            iters = max(iters, 1)
    ...
    def _build_loss(self, loss_name, loss_cfg: dict):
        ...
        def _check_output_index(output_index):
            ...
            if all_index != target:
                raise ValueError(
                    "The values of `output_index` must be continuous intergers")
        ...
        loss_dict["output_index"] = loss_cfg.get(
            "output_index", _get_default_output_index(loss_cfg))
        _check_output_index(loss_dict["output_index"])
        if len(loss_dict["output_index"]) != len(loss_dict["types"]):
            raise ValueError(
                "The length of `output_index` and `types` must be equal.")
        return loss_dict

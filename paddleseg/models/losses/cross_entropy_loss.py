...
class CrossEntropyLoss(nn.Layer):
    ...
    def forward(self, logit, label, semantic_weights=None):
        ...
        loss = self._post_process_loss(logit, label, semantic_weights, loss)
        return loss
...
class DistillCrossEntropyLoss(CrossEntropyLoss):
    """
    Implements the distill cross entropy loss function.
    Based on the ground truth label and teacher label, it calculates the combined loss.

    Args:
        weight (tuple|list|ndarray|Tensor, optional): A manual rescaling weight
            given to each class. Its length must be equal to the number of classes.
            Default ``None``.
        alpha (float, optional):  Alpha should in [0.0, 1.0].
    ...
    def __init__(self,
        ...
        if alpha < 0. or alpha > 1.:
            raise ValueError(
                'The alpha = {} should in [0.0, 1.0]'.format(alpha))
        self.alpha = alpha
    ...
    def forward(self,
        ...
        loss_gt = super().forward(student_logit, label, semantic_weights)
        ...
        teacher_logit = paddle.nn.functional.softmax(teacher_logit)
        ...
        loss_distill = self._post_process_loss(student_logit, label,
                                               semantic_weights, loss_distill)

        return self.alpha * loss_gt + (1 - self.alpha) * loss_distill

...
from paddle.nn import AdaptiveAvgPool2D, Conv2D, Dropout, Linear
...
class PPLCNet(nn.Layer):
    def __init__(self,
        ...
        self.feat_channels = [make_divisible(16 * scale)]
        for i in range(2, 6):
            self.feat_channels.append(NET_CONFIG["blocks{}".format(i)][-1][2])
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)  # 1/2
        feat_list = [x]
        blocks = [self.blocks2, self.blocks3, self.blocks4, self.blocks5]
        for block in blocks:
            x = block(x)
            feat_list.append(x)
        ...
        # x = self.blocks2(x) # 1/2
        # x = self.blocks3(x) # 1/4
        # x = self.blocks4(x) # 1/8
        # x = self.blocks5(x) # 1/16
        # # x = self.blocks6(x) # 1/32
...
def PPLCNet_x1_0(pretrained=True, use_ssld=True, **kwargs):
    """
    PPLCNet_x1_0
    Args:
        pretrained: bool=False or str. If `True` load pretrained parameters, `False` otherwise.
    ...
    """
    if pretrained:
        pretrained = MODEL_URLS["PPLCNet_x1_0"]
    if use_ssld:
        pretrained = pretrained.replace("_pretrained", "_ssld_pretrained")
    model = PPLCNet(scale=1.0, pretrained=pretrained, **kwargs)
    return model

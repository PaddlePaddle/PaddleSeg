...
class NonLocal2d(nn.Layer):
    ...
    def __init__(self, in_channels, reduction=2, use_scale=True, sub_sample=False, mode='embedded_gaussian'):
        ...
        if mode not in ['gaussian', 'embedded_gaussian', 'dot_product', 'concatenation']:
            raise ValueError("Mode should be in 'gaussian', 'concatenation','embedded_gaussian' or 'dot_product'.")

        self.inter_channels = max(in_channels // reduction, 1)
        
        self.g = nn.Conv2D(in_channels=self.in_channels, out_channels=self.inter_channels, kernel_size=1)
        self.conv_out = layers.ConvBNReLU(in_channels=self.inter_channels, out_channels=self.in_channels, kernel_size=1, bias_attr = False)
        ...
        if self.mode == "concatenation":
            self.concat_project = layers.ConvBNReLU(in_channels=self.inter_channels * 2, out_channels=1, kernel_size=1, bias_attr = False)

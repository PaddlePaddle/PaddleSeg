...
class RandomCrop:
    ...
    def image_in_crop(self, target_w, target_h, w, h, img, mask):
        ...
        slid_image = self.add_margin(img, top, right, bottom, left,
                                     self.pad_color)
        slid_mask = self.add_margin(mask, top, right, bottom, left,
                                    self.ignore_index)
    ...
    def __call__(self, img, mask, centroid=None):
        ...
        if self.nopad:
            ...
            if pad_h or pad_w:
                ...
                w, h = img.size
        im, label = self.crop_in_image(centroid, target_w, target_h, w, h, img,
                                       mask)
        return (im, label)
...
class RandomSizeAndCrop(object):
    ...
    def __call__(self, img, mask, centroid=None):
        ...
        img_mask = list(self.crop(resized_img, resized_mask, centroid))
        img_mask.append(scale_amt)
        im, label = np.array(
            img_mask[0], dtype='float32'), np.array(
                img_mask[1], dtype='float32')
        return (im, label)

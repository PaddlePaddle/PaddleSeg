...
class ResizeStepScaling:
    ...
    Args:
        ...
        base_size (float, optional): base_size of image. Default: 1024.
        resize_by_long (bool, optional): Whether resize images by long side. Default: False.
    ...
    def __init__(
        ...
        self.resize_by_long = resize_by_long
    ...
    def __call__(self, data):
        ...
        if self.resize_by_long:
            long_size = int(self.base_size * scale_factor + 0.5)
            ...
            new_w = int(round(scale_factor * data["img"].shape[1]))
            new_h = int(round(scale_factor * data["img"].shape[0]))

        data["img"] = functional.resize(data["img"], (new_w, new_h),
                                        cv2.INTER_LINEAR)
        for key in data.get("gt_fields", []):
            data[key] = functional.resize(data[key], (new_w, new_h),
                                          cv2.INTER_NEAREST)
...
class Normalize:
    ...
    def __init__(self, mean=(0.5, ), std=(0.5, )):
        if (not (isinstance(mean,
                            (list, tuple)) and isinstance(std,
                                                          (list, tuple))) and
            (len(mean) not in [1, 3]) and (len(std) not in [1, 3])):
            raise ValueError(
...
class RandomPaddingCrop:
    ...
    def __init__(
        ...
        self.im_padding_value = im_padding_value
        self.label_padding_value = label_padding_value
        self.gt_fields_padding_value = gt_fields_padding_value or label_padding_value
        ...
        if pad_height > 0 or pad_width > 0:
            ...
            for key in data.get("gt_fields", []):
                data[key] = cv2.copyMakeBorder(
                    ...
                    pad_width,
                    cv2.BORDER_CONSTANT,
                    value=self.gt_fields_padding_value, )
        return data
        ...
        if isinstance(retain_ratio, list) or isinstance(retain_ratio, tuple):
            if len(retain_ratio) != 2:
                raise ValueError(
                    ...
                    format(retain_ratio))
            if (retain_ratio[0] > 1 or retain_ratio[1] > 1 or
                    retain_ratio[0] < 0 or retain_ratio[1] < 0):
                raise ValueError(
...
class AddMultiLabelAuxiliaryCategory:
    ...
    def __call__(self, data):
        if 'label' in data:
            aux_label = (
                data['label'].sum(axis=-1, keepdims=True) == 0).astype('uint8')

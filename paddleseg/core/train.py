...
from paddleseg.utils import (TimeAverager, calculate_eta, logger, op_flops_funs,
                             resume, train_profiler, worker_init_fn)
...
def loss_computation(logits_list, labels, edges, losses):
    ...
    for i in range(len(losses["types"])):
        ...
        index_i = losses["output_index"][i]
        inputs = [logits[i] for i in index_i]
        if loss_i.__class__.__name__ in ("BCELoss", ) and loss_i.edge_label:
            ...
            loss_list.append(coef_i * loss_i(logits_list[0],
                                             logits_list[1].detach()))
        else:

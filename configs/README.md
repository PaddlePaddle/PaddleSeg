...
- The table shows some evaluation parameters of the segmentation model implemented by PaddleSeg under the configuration that achieves the highest classification accuracy.
- Among them, mIoU, mIoU(flip), mIoU(ms+flip) are the results of evaluating the model.
- `flip` means to flip the original image horizontally, `ms` means `multi-scale`, which is to scale the original image at a ratio of 0.75, 1.0, 1.25.
- The inference time is the average result of 100 predictions using the images in the CityScapes dataset.

...
class APP_EISeg(QMainWindow, Ui_EISeg):
    ...
    def initActions(self):
        ...
        video_recent_params = newWidget(self.tr("近期视频传播模型及参数"), "Net", self.updateVideoModelMenu)
    ...
    def updateVideoModelMenu(self):
        ...
        for idx, m in enumerate(self.video_recentModels):
            ...
            action.triggered.connect(partial(self.setVideoModelParam, m["param_path"]))
            menu.addAction(action)
    ...
    def changePropgationParam(self, param_path: str = None):
        if not param_path:
            filters = self.tr("Paddle静态模型权重文件(*.pdiparams)")
            start_path = (".") if len(self.video_recentModels) == 0 else osp.dirname(self.video_recentModels[-1]["param_path"])
            if self.settings.value("use_qt_widget", False, type=bool):
                options = QtWidgets.QFileDialog.DontUseNativeDialog
            else:
                options = QtWidgets.QFileDialog.ReadOnly
        ...
        if success:
            ...
            if model_dict not in self.video_recentModels:
                ...
                self.video_recentModels.insert(0, model_dict)
            self.settings.setValue("video_recent_models", self.video_recentModels)
    ...
    def openImage(self, filePath: str = None):
        ...
        if not isinstance(filePath, str) or filePath is False:
            ...
            if osp.splitext(filePath)[-1] in self.video_ext:
                if not paddle.device.is_compiled_with_cuda():  # TODO: 可以使用GPU却返回False
                    self.warn(self.tr("请在gpu电脑上进行视频标注"),
    ...
    def loadImage(self, path):
        ...
        if path.lower().endswith(tuple(self.formats[3])):  # mp4
            ...
            self.video_masks = np.zeros((self.video.num_frames, self.video.height, self.video.width), dtype=np.uint8)
    ...
    def loadLabel(self, imgPath):
        ...
        if self.video_images is not None:
            ...
            for cursur in range(self.video.num_frames):
                ...
                pseudo = cv2.imread(osp.join(maskPath, '{:05d}.png'.format(cursur)))
    ...
    def reset_video(self):
        ...
        self.videoPlay.setIcon(QtGui.QIcon(osp.join(pjpath, "resource/Play.png")))
        self.ratio = 20
        ...
        if self.video_masks is not None:
            ...
            if osp.isdir(savePath):
                ...
                for i in range(0, self.video.num_frames):
                    ...
                    cv2.imwrite(os.path.join(mask_dir, '{:05d}.png'.format(i)), pseudo)

                    # Save overlay
                    overlay = overlay_davis(self.video_images[i], self.video_masks[i], self.opacity,
                ...
                self.setDirty(False)
                self.statusbar.showMessage(self.tr("视频帧成功保存至") + " " + savePath, 5000)
                return
    ...
    def show_current_frame(self):
        self.viz = overlay_davis(self.video_images[self.video.cursur], self.video_masks[self.video.cursur],
    ...
    def on_play(self):
        ...
        if self.timer.isActive():
            ...
            self.videoPlay.setIcon(QtGui.QIcon(osp.join(pjpath, "resource/Stop.png")))
    ...
    def on_propgation(self):
        ...
        self.video_masks = self.video.interact(
            self.one_hot_mask, self.video.cursur, self.progress_total_cb, self.progress_step_cb
        )
        end = time.time()
        ...
        if self.TDDock.isVisible():
            self.vtkWidget.show_array(np.uint8(self.video_masks), (1., 1., 1.), color_map)

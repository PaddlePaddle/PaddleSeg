...
def train(model,
    ...
    while iter < iters:
        for data in loader:
            ...
            val_dataset0, val_dataset1, val_dataset2, val_dataset3 = val_dataset
            if (iter % save_interval == 0
                ...
                mean_iou, acc, class_iou3, _, _ = evaluate(
                    model, val_dataset3, num_workers=num_workers)
                model.train()
            ...
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                ...
                if val_dataset is not None:
                    ...
                    dataset3_iou = class_weight[0] * class_iou3[
                        0] + class_weight[1] * class_iou3[1]
                    total_iou = dataset_weight[
                        0] * dataset0_iou + dataset_weight[
                            1] * dataset1_iou + dataset_weight[
                                2] * dataset2_iou + dataset_weight[
                                    3] * dataset3_iou
                    ...
                    logger.info("[EVAL] Dataset 3 Class IoU: \n" +
                                str(np.round(class_iou3, 4)))
                    ...
                    logger.info("[EVAL] Dataset 3 Class IoU: \n" +
                                str(np.round(best_class_iou3, 4)))
                    logger.info("[EVAL] Total IoU: \n" +
                                str(np.round(total_iou, 4)))
                    logger.info(
                    ...
                    if use_vdl:
                        ...
                        for k, class_iou in enumerate(
                            [class_iou0, class_iou1, class_iou2, class_iou3]):

...
class PromptEncoder(paddle.nn.Layer):
    ...
    def __init__(self, embed_dim: int, image_embedding_size: Tuple[int, int
        ...
        point_embeddings = [paddle.nn.Embedding(1, embed_dim) for i in range
        ...
        self.mask_downscaling = paddle.nn.Sequential(paddle.nn.Conv2D(
            in_channels=1, out_channels=mask_in_chans // 4, kernel_size=2,
            stride=2), LayerNorm2d(mask_in_chans // 4), activation(),
            paddle.nn.Conv2D(in_channels=mask_in_chans // 4, out_channels=
            mask_in_chans, kernel_size=2, stride=2), LayerNorm2d(
            mask_in_chans), activation(), paddle.nn.Conv2D(in_channels=
            mask_in_chans, out_channels=embed_dim, kernel_size=1))
    ...
    def _embed_points(self, points: paddle.Tensor, labels: paddle.Tensor,
    ...
    def _get_batch_size(self, points: Optional[Tuple[paddle.Tensor, paddle.
        Tensor]], boxes: Optional[paddle.Tensor], masks: Optional[paddle.
        Tensor]) ->int:
        """
    ...
    def forward(self, points: Optional[Tuple[paddle.Tensor, paddle.Tensor]],
        boxes: Optional[paddle.Tensor], masks: Optional[paddle.Tensor]
        ...
        if points is not None:
            ...
            point_embeddings = self._embed_points(coords, labels, pad=boxes is
                None)
            sparse_embeddings = paddle.concat(x=[sparse_embeddings,
                point_embeddings], axis=1)
        if boxes is not None:
            box_embeddings = self._embed_boxes(boxes)
            sparse_embeddings = paddle.concat(x=[sparse_embeddings,
                box_embeddings], axis=1)
        if masks is not None:
            dense_embeddings = self._embed_masks(masks)
        else:
            """Class Method: *.reshape, not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*, and convert manually"""
            dense_embeddings = self.no_mask_embed.weight.reshape([1, -1, 1, 1]
                ).expand(shape=[bs, -1, self.image_embedding_size[0], self.
...
class PositionEmbeddingRandom(paddle.nn.Layer):
    ...
    def _pe_encoding(self, coords: paddle.Tensor) ->paddle.Tensor:
        ...
        return paddle.concat(x=[paddle.sin(x=coords), paddle.cos(x=coords)],
            axis=-1)
    ...
    def forward_with_coords(self, coords_input: paddle.Tensor, image_size:

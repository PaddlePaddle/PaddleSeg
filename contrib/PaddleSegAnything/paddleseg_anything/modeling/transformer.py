...
class TwoWayTransformer(nn.Layer):
    ...
    def __init__(self, depth: int, embedding_dim: int, num_heads: int,
        mlp_dim: int, activation: Type[nn.Layer]=nn.ReLU,
        ...
        for i in range(depth):
            self.layers.append(TwoWayAttentionBlock(embedding_dim=
                embedding_dim, num_heads=num_heads, mlp_dim=mlp_dim,
                activation=activation, attention_downsample_rate=
    ...
    def forward(self, image_embedding, image_pe, point_embedding) ->Tuple[
        Tensor, Tensor]:
        """
        Args:
        ...
        for layer in self.layers:
            queries, keys = layer(queries=queries, keys=keys, query_pe=
                point_embedding, key_pe=image_pe)
...
class TwoWayAttentionBlock(nn.Layer):
    ...
    def __init__(self, embedding_dim: int, num_heads: int, mlp_dim: int=
        2048, activation: Type[nn.Layer]=nn.ReLU, attention_downsample_rate:
        int=2, skip_first_layer_pe: bool=False) ->None:
        """

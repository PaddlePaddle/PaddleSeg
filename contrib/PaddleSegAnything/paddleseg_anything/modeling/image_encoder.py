...
class ImageEncoderViT(nn.Layer):
    def __init__(
        ...
        if use_abs_pos:
            ...
            self.pos_embed = paddle.create_parameter(shape=[1, img_size // patch_size, img_size // patch_size, embed_dim], dtype='float32', default_initializer=nn.initializer.Constant(value=0.0))
...
class Attention(nn.Layer):
    ...
    def __init__(
        ...
        if self.use_rel_pos:
            ...
            self.rel_pos_h = paddle.create_parameter(shape=[2 * input_size[0] - 1, head_dim], dtype='float32', default_initializer=nn.initializer.Constant(value=0.0))
            self.rel_pos_w = paddle.create_parameter(shape=[2 * input_size[1] - 1, head_dim], dtype='float32', default_initializer=nn.initializer.Constant(value=0.0))
...
if __name__ == '__main__':
    ...
    inp = ({'x': paddle.to_tensor(random_numbers)}, 
            {'x': torch.as_tensor(random_numbers)})

    auto_diff(image_encoder, image_encoder_t, inp, auto_weights=True, options={'atol': 0.001, 'rtol':0, 'compare_mode': 'mean', 'single_step':False})

...
from paddleseg.core.train_assit import predict_with_out_grad
...
def aug_inference(model,
    ...
    h_input, w_input = label.shape[-2], label.shape[-1]

    h = ceil(h_input / 8) * 8
    w = ceil(w_input / 8) * 8
    # resize to network acceptable
    im = F.interpolate(im, [h, w], mode='bilinear', align_corners=True)
    output1, output2 = predict_with_out_grad(model, im)
    output = (output1 + output2) / 2
    pre = paddle.nn.functional.interpolate(output,
...
def semi_inference(model,
    ...
    Returns:
        Tensor: If ori_shape is not None, a prediction with shape (1, 1, h, w) is returned.
            If ori_shape is None, a logit with shape (1, num_classes, h, w) is returned.
    """
    im = im.cuda(blocking=False)
    H, W = im.shape[-2], im.shape[-1]
    up_sizes = (ceil(H / 8) * 8, ceil(W / 8) * 8)
    data = paddle.nn.functional.interpolate(im,
                                            size=(up_sizes[0], up_sizes[1]),
                                            mode='bilinear',
                                            align_corners=True)
    with paddle.no_grad():
        prediction = model.decoder1(model.encoder1(data),
                                    data_shape=[data.shape[-2], data.shape[-1]])
    prediction = paddle.nn.functional.interpolate(prediction,
                                                  size=(H, W),
                                                  mode='bilinear',
                                                  align_corners=True)
    return prediction

...
def batch_pix_accuracy(output, target):
    predict = paddle.argmax(output, 1)
    predict = paddle.cast(predict, dtype="int32") + 1
    target = paddle.cast(target, dtype="int32") + 1
...
def batch_intersection_union(output, target, num_class):
    ...
    predict = paddle.cast(predict, dtype="int64")
    intersection = predict * (predict == target)
    intersection = paddle.cast(intersection, dtype="int64")
    # area_inter交的区域，area_union并的区域
    area_inter = paddle.histogram(paddle.cast(intersection, dtype="float32"), bins=num_class, max=num_class, min=1)
    area_pred = paddle.histogram(paddle.cast(predict, dtype="float32"), bins=num_class, max=num_class, min=1)
    area_lab = paddle.histogram(paddle.cast(target, dtype="float32"), bins=num_class, max=num_class, min=1)
...
def eval_metrics(output, target, num_classes, ignore_index):
    ...
    inter, union = batch_intersection_union(output, target, num_classes)
...
def get_seg_metrics(total_correct, total_label, total_inter, total_union):
    pixAcc = 1.0 * total_correct / (np.spacing(1) + total_label)
    IoU = 1.0 * total_inter / (np.spacing(1) + total_union)

    mIoU = IoU.mean()
    return np.round(pixAcc, 3), np.round(IoU, 3), np.round(mIoU, 3)

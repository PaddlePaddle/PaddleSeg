...
class PascalVOC(Dataset):
    ...
    def __init__(self, transforms, dataset_root=None, mode='train', edge=False, n_label=732):
        ...
        self.edge = edge
        self.n_labeled_examples = n_label

        if mode not in ['train', 'trainval', 'trainaug', 'val', 'train_sup', 'train_unsup']:
            raise ValueError(
        ...
        if mode == 'train':
            ...
            file_path = os.path.join(image_set_dir, f"{self.n_labeled_examples}_train_supervised" + ".txt")
        elif mode == 'train_unsup':
            file_path = os.path.join(image_set_dir, f"{self.n_labeled_examples}_train_unsupervised" + ".txt")
        elif mode == 'val':
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                img_id = line[0].split("/")[-1].split(".")[0]

                image_path = os.path.join(img_dir, ''.join([img_id, '.jpg']))
                label_path = os.path.join(label_dir, line[1][1:])

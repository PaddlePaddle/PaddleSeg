...
class resize:
    ...
    def __call__(self, data):
        ...
        data['img'] = np.asarray(Image.fromarray(np.uint8(im)).resize((w, h), Image.BICUBIC))
        data['label'] = cv2.resize(label, (w, h), interpolation=cv2.INTER_NEAREST)
        return data
...
class RandomGrayscale(BaseTransform):
    ...
    def __init__(self, p=0.1):
        super(RandomGrayscale).__init__()
        self.p = p
...
class data_aug:
    def __init__(self, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
        self.kernel_size = int(random.random() * 4.95)
    ...
    def __call__(self, data):
        weak_aug = data['img']
        weak_aug = functional.normalize(np.array(weak_aug), mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))

        strong_aug = data['img']
        strong_aug = Image.fromarray(np.uint8(strong_aug))
        if random.random() < 0.8:
            strong_aug = self.color_jitter(strong_aug)
        strong_aug = RandomGrayscale(p=0.2)(strong_aug)

        if random.random() < 0.5:
            # 将PIL格式变为cv2
            strong_aug = cv2.cvtColor(np.asarray(strong_aug), cv2.COLOR_RGB2BGR)

            strong_aug = cv2.GaussianBlur(strong_aug, (self.kernel_size, self.kernel_size), sigmaX=0.1, sigmaY=2.0)
            # 将cv2变为PIL
            strong_aug = Image.fromarray(cv2.cvtColor(strong_aug, cv2.COLOR_BGR2RGB))

        strong_aug = functional.normalize(np.array(strong_aug), mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225))
        data['weak_aug'] = np.transpose(weak_aug, (2, 0, 1))
        data['strong_aug'] = np.transpose(strong_aug, (2, 0, 1))
        return data

...
class Config(object):
                ...
                for item in args['types']:
                    if item['type'] != 'MixedLoss':
                        if 'ignore_index' in item:
                            assert item[
                                       'ignore_index'] == self.train_sup_dataset.ignore_index, 'If ignore_index of loss is set, ' \
                                ...
                                item['ignore_index'], self.train_sup_dataset.ignore_index)
                        item['ignore_index'] = \
                            self.train_sup_dataset.ignore_index
    ...
    @property
    def train_sup_dataset_config(self) -> Dict:
        return self.dic.get('train_sup_dataset', {}).copy()

    @property
    def train_unsup_dataset_config(self) -> Dict:
        return self.dic.get('train_unsup_dataset', {}).copy()
    ...
    @property
    # 监督数据集
    def train_sup_dataset(self) -> paddle.io.Dataset:
        _train_dataset = self.train_sup_dataset_config
        if not _train_dataset:
            return None
        return self._load_object(_train_dataset)
    ...
    @property
    # 无监督数据集
    def train_unsup_dataset(self) -> paddle.io.Dataset:
        _train_dataset = self.train_unsup_dataset_config
        if not _train_dataset:
            return None
        return self._load_object(_train_dataset)

...
class MixedDataConfig(object):
    ...
    @property
    def train_roots(self):
        return self.dic.get('train_roots')

    @property
    def train_file_lists(self):
        return self.dic.get('train_file_lists',
                            ['train.txt', 'train.txt', 'train.txt'])

    @property
    def train_data_ratio(self):
        return self.dic.get('train_data_ratio')

    @property
    def val_roots(self):
        return self.dic.get('val_roots')

    @property
    def dataset_weights(self):
        return self.dic.get('dataset_weights')

    @property
    def class_weights(self):
        return self.dic.get('class_weights')

    @property
    def train_transforms_config(self) -> Dict:
        return self.dic.get('train_transforms', {}).copy()

    @property
    def val_transforms_config(self) -> Dict:
        return self.dic.get('val_transforms', {}).copy()

    @property
    def train_transforms(self) -> list:
        _train_transforms = self.train_transforms_config
        if not _train_transforms:
            return None
        out = []
        for transform in _train_transforms:
            out.append(self._load_object(transform))
        return out

    @property
    def val_transforms(self) -> list:
        _val_transforms = self.val_transforms_config
        if not _val_transforms:
            return None
        out = []
        for transform in _val_transforms:
            out.append(self._load_object(transform))
        return out
    ...
    def save_interval(self) -> int:
        return self.dic.get('save_interval', 1000)

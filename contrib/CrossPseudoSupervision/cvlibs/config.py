...
class Config(object):
    ...
    def nepochs(self) -> int:
        return self.dic.get('nepochs')

    @property
    def labeled_ratio(self) -> int:
        return self.dic.get('labeled_ratio')
    ...
    @property
    def lr_scheduler_l_cfg(self) -> Dict:
        return self.dic.get('lr_scheduler_l', {}).copy()

    @property
    def lr_scheduler_r_cfg(self) -> Dict:
        return self.dic.get('lr_scheduler_r', {}).copy()

    @property
    def optimizer_l_cfg(self) -> Dict:
        return self.dic.get('optimizer_l', {}).copy()

    @property
    def optimizer_r_cfg(self) -> Dict:
        return self.dic.get('optimizer_r', {}).copy()
    ...
    @property
    def unsupervised_train_dataset_cfg(self) -> Dict:
        return self.dic.get('unsupervised_train_dataset', {}).copy()
...
def update_config_dict(dic: dict,
    ...
    if learning_rate:
        dic['lr_scheduler_r']['learning_rate'] = learning_rate
        dic['lr_scheduler_l']['learning_rate'] = learning_rate
    if batch_size:
    ...
    if labeled_ratio:
        dic['labeled_ratio'] = labeled_ratio

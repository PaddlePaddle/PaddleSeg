...
def generate_sliding_windows(ndarray_data,window_size):
    ...
    assert (w_z<=z)and(w_y<=y)and(w_x<=x),"window_size should smaller than the size of image"
    sliding_windows=[]
    for z_index in range(0,z,w_z):
        ...
        for y_index in range(0,y,w_y):
            ...
            for x_index in range(0,x,w_x):
                if x_index+w_x>x:
                    x_index=max(x-w_x,0)
                sliding_windows.append([[z_index,z_index+w_z],[y_index,y_index+w_y],[x_index,x_index+w_x]])

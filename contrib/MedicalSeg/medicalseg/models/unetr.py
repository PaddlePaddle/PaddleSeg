...
class UNETR(nn.Layer):
    ...
    def forward(self, x):
        ...
        # z3, z6, z9, z12 = map(lambda t: rearrange(t, 'b (x y z) d -> b d x y z',x=self.patch_dim[0], y=self.patch_dim[1], z=self.patch_dim[2]),
        ...
        # Concat + yellow conv
        y = torch.concat([z12, z9], axis=1)
        y = self.z9_conv(y)

        # Green block for z6
        y = self.z9_deconv(y)
        # Concat + yellow conv
        y = torch.concat([y, z6], axis=1)
        y = self.z6_conv(y)

        # Green block for z3
        y = self.z6_deconv(y)
        # Concat + yellow conv
        y = torch.concat([y, z3], axis=1)
        y = self.z3_conv(y)

        y = self.z3_deconv(y)
        y = torch.concat([y, z0], axis=1)

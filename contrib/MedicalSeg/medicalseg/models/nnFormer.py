...
class Patch_Expanding(nn.Layer):
    def __init__(self, dim, norm_layer=nn.LayerNorm, tag=None):
        super().__init__()
        self.dim = dim
        self.norm = norm_layer(dim)
        if tag == 0:
            ...
            self.up = nn.Conv3DTranspose(dim, dim // 2, [2, 2, 2], [2, 2, 2], output_padding=[1, 0, 0])
...
def window_partition(x,window_size):
    B, S, H, W, C = x.shape
    x=x.reshape([B, S // window_size[0], window_size[0],
                 H // window_size[1], window_size[1],
                 W // window_size[2], window_size[2], C])# [bs,num_window,window_size,num_window,window_size,num_window,window_size,C]
    x=x.transpose([0,1,3,5,2,4,6,7])# [bs,num_window,num_window,num_window,window_size,window_size,window_Size,C]
    x = x.reshape([-1, window_size[0], window_size[1], window_size[2],
                   C])  # [bs*num_window*num_window*num_window,window_size,window_size,window_Size,C]
    return x


def window_reverse(windows, window_size, S, H, W):
    B = int(windows.shape[0] / (S * H * W / window_size[0] / window_size[1] / window_size[2]))
    x = windows.reshape((B, S // window_size[0], H // window_size[1], W // window_size[2], window_size[0], window_size[1],
                     window_size[2], -1))
...
class WindowAttention_kv(nn.Layer):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = paddle.create_parameter(
            shape=[(2 * window_size[0] - 1) * (2 * window_size[1] - 1)*(2 * window_size[2] - 1), num_heads],
            dtype='float32',
        ...
        #there is a bug in reference code
    ...
    def forward(self, skip, x_up, mask=None):
        ...
        kv = kv.reshape((B_, N, 2, self.num_heads, C // self.num_heads)).transpose((2, 0, 3, 1, 4))
        #kv 2*B*numheads*N*channels
        q = q.reshape((B_, N, self.num_heads, C // self.num_heads)).transpose((0, 2, 1, 3))
        k, v = kv[0], kv[1]
        q = q * self.scale
...
class WindowAttention(nn.Layer):
    ...
    def forward(self, x, mask=None, pos_embed=None):
        B_, N, C = x.shape
        qkv = self.qkv(x)
        qkv=qkv.reshape([B_,N,3,self.num_heads,C//self.num_heads]).transpose([2, 0, 3, 1, 4])
...
class SwinTransformerBlock_kv(nn.Layer):
    ...
    def forward(self, x, mask_matrix,skip=None,x_up=None):
        ...
        skip = skip.reshape((-1, self.window_size[0] * self.window_size[1] * self.window_size[2],
                                   C))
        x_up = window_partition(x_up, self.window_size)
        x_up = x_up.reshape((-1, self.window_size[0] * self.window_size[1] * self.window_size[2],
                                   C))
        # W-MSA/SW-MSA
        ...
        if min(self.shift_size) > 0:
            x = paddle.roll(shifted_x, shifts=(self.shift_size[0], self.shift_size[1], self.shift_size[2]), axis=(1, 2, 3))
        else:
            x = shifted_x
...
class SwinTransformerBlock(nn.Layer):
    ...
    def forward(self, x,mask_matrix):
        ...
        # cyclic shift
        if min(self.shift_size) > 0:
            shifted_x = paddle.roll(x,shifts=[-self.shift_size[0], -self.shift_size[1],-self.shift_size[2]],axis=[1,2,3])
        ...
        # [bs*num_window*num_window*num_window,window_size,window_size,window_Size,C]
        x_windows = x_windows.reshape((-1, self.window_size[0] * self.window_size[1] * self.window_size[2],
                                   C))
...
class project(nn.Layer):
    ...
    def forward(self, x):
        x=self.conv1(x)
        x = self.activate(x)
        #norm x.shape [batch,out_dim,s,h,w]
...
class PatchEmbed(nn.Layer):
    ...
    def forward(self,x):
        #padding
        _,_,S,H,W=x.shape
        #W为最后一个维度，确认最后一个维度能被patch最后一个维度整除
        ...
        if S % self.patch_size[0] != 0:
            x = F.pad(x, (0, 0, 0, 0, 0, self.patch_size[0] - S % self.patch_size[0]),data_format='NCDHW')
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            ...
            if i in self.out_indices:
                ...
                out = x_out.reshape((-1, S, H, W, self.num_features[i])).transpose((0, 4, 1, 2, 3))

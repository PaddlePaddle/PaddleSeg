...
class VNetDeepSup(nn.Layer):
    ...
    def forward(self, x):
        ...
        d1 = F.interpolate(
            d1, size=x.shape[2:], data_format='NCDHW', mode='trilinear')
        out = self.up_tr128(out, out64)  # [4, 128, 128, 128, 8]
        d2 = self.out_tr128(out)
        d2 = F.interpolate(
            d2, size=x.shape[2:], data_format='NCDHW', mode='trilinear')
        out = self.up_tr64(out, out32)  # [4, 64, 256, 256, 9]
        d3 = self.out_tr64(out)
        d3 = F.interpolate(
            d3, size=x.shape[2:], data_format='NCDHW', mode='trilinear')

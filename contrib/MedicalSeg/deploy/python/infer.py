...
class DeployConfig:
    def __init__(self, path, ig_max=False):
        with codecs.open(path, 'r', 'utf-8') as file:
            self.dic = yaml.load(file, Loader=yaml.FullLoader)

        self._transforms = self.load_transforms(
            self.dic['Deploy']['transforms'], ig_max)
    ...
    @staticmethod
    def load_transforms(t_list, ig_max=False):
        ...
        return T.Compose(transforms, igmax=ig_max)
...
class Predictor:
    def __init__(self, args):
        ...
        """
        self.args = args
        self.cfg = DeployConfig(args.cfg, args.ig_max)

...
class PreProcessor:
    ...
    def process(self, image_file):
        ...
        ori_w = im.shape[1]
        if channels == 1:
            im = cv2.cvtColor(im, dstCn=cv2.COLOR_GRAY2BGR)
            channels = im.shape[2]
        ...
        im -= im_mean
        im /= im_std
        # print("After preprocessing image, top 10 values:")
        # print(im[0, 0, :10])
...
class Predictor:
    ...
    def make_tensor(self, inputs, batch_size):
        ...
        im_tensor.data = PaddleBuf(inputs.flatten().astype("float32").tolist())

        return [im_tensor]

    def output_result(self, image_name, output, ori_h, ori_w):
        ...
        result_png = cv2.resize(score_png, (ori_w, ori_h), fx=0, fy=0, interpolation=cv2.INTER_CUBIC)
    ...
    def predict(self, images):
        ...
        for i in range(0, len(images), batch_size):
            ...
            ori_w_list = []
            for j in range(bz):
                im, ori_h, ori_w = self.preprocessor.process(images[i + j])
                im_list.append(im)
                ori_h_list.append(ori_h)
                ori_w_list.append(ori_w)
            input_data = np.concatenate(im_list)
            input_data = self.make_tensor(input_data, bz)
            start = time.time()
            output_data = self.predictor.run(input_data)[0]
            print("runtime = %f " % ((time.time() - start) * 1000000))
            output_data = output_data.as_ndarray()
            # print("Top 10 output values:")
            # print(output_data[0, 0, 0, :10])
            for j in range(bz):
                self.output_result(images[i + j], output_data[j],
                                 ori_h_list[j], ori_w_list[j])

...
class MLFF(nn.Layer):
    ...
    Args:
        ...
        mid_channles(list): The middle channels whiling fusing the features.
        out_channel(int): The output channel after fusing.
        merge_type(str): Which type to merge the multi features before output. It should be one of ('add', 'concat'). Default: 'concat'.
    """
    ...
    def __init__(self,
        ...
        if self.merge_type == 'add':
            if len(np.unique(np.array(mid_channels))) == 1:
                in_chan = mid_channels[0]
            else:
                raise ValueError(
                    "if `merge_type='add', mid_channels should be same of all input features"
                )
        else:

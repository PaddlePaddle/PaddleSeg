...
from ppmatting.cvlibs import param_init
...
class GuidedCxtAtten(nn.Layer):
    ...
    def compute_similarity_map(self, img_feat, img_ps):
        ...
        escape_NaN = paddle.to_tensor([1e-4])
        # img_ps_normed = img_ps / paddle.max(self.l2_norm(img_ps), escape_NaN)
...
if __name__ == "__main__":
    ...
    paddle.seed(0)
    gca = GuidedCxtAtten(128, 128)

    img_feat = paddle.to_tensor(
        np.random.random([1, 128, 64, 64]).astype(np.float32))

    alpha_feat = paddle.to_tensor(
        np.random.random([1, 128, 64, 64]).astype(np.float32))

    unknown = paddle.to_tensor(
        np.random.random([1, 1, 64, 64]).astype(np.float32))

    print("a", img_feat.mean())
    print("b", alpha_feat.mean())
    print("c", unknown.mean())

    y = gca(img_feat, alpha_feat, unknown)
    print(y.mean())

...
class GCABaseline(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        alpha_pred, _ = self.decoder(embedding, mid_fea)
...
if __name__ == "__main__":
    ...
    from backbone import gca_enc

    encoder = gca_enc.ResShortCut_D(6, [3, 4, 4, 2])
    decoder = ResShortCut_D_Dec([2, 3, 3, 2])

    inp = paddle.ones((1, 6, 512, 512))
    embedding, mid_fea = encoder(inp)
    alpha, info_dict = decoder(embedding, mid_fea)

    print(alpha.shape)
    # m(paddle.ones((1, 512, 16, 16)))

    # info = paddle.summary(m, (1, 512, 16, 16))
    # print(info)

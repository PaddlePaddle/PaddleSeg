...
if __name__ == "__main__":
    ...
    m = ResNet_D(4, [3, 4, 4, 2], pretrained="gca_encoder.pdparams")
    # info = paddle.summary(m, (1, 6, 512, 512))
    # print(info)

    # torch_list = []
    # with open("resnet_enc_torch.txt") as f:
    #     for line in f.readlines():
    #         items = line.split(' ')
    #         if "num_batches_tracked" not in items[0]:
    #             torch_list.append(items[0]) 
    ...
    # set_torch = set(torch_list)
    # set_paddle = set(state_dict.keys())

    # A_B = set_torch.difference(set_paddle)
    # print(A_B, len(A_B))

    # print("--------------------------------")
    # B_A = set_paddle.difference(set_torch)
    # print(B_A, len(B_A))

    # print("--------------------------------")
    # AB = set_paddle.intersection(set_torch)
    # print(len(AB))

    # import pickle

    # filename = "res_encoder_torch_4.pickle"
    ...
    # m.set_dict(state_dict)

    # layer_state_dict = m.state_dict()
    # paddle.save(layer_state_dict, "gca_encoder.pdparams")

    m.eval()
    x = m(paddle.ones((1, 4, 512, 512)))
    print(paddle.mean(x[0]))
